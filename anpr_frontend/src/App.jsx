...
const ANPRApp = () => {
  ...
  const handleProceed = async () => {
    if (!photo1 && !photo2) {
      alert("Please upload at least one vehicle photo before proceeding.");
      return;
    }
    setIsLoading(true);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-white flex flex-col items-center justify-center p-6">
      ...
      <div className="flex flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-14 mb-10">
      ...
      <div className="mb-10 px-10 py-3 w-36 bg-indigo-600 text-white text-lg font-medium rounded-lg hover:bg-indigo-700 transition duration-200 shadow-md">
        {isLoading ? (
          <Loader />
        ) : (
        ...
        <div className="flex flex-col items-center space-y-8">
            ...
            {photo1 && (
              <div className="border border-indigo-400 rounded-lg p-6 text-center bg-white shadow-md w-72">
                ...
                <p className="text-indigo-600 text-sm">
                  Extracted Text: {resultData.detected_number1}
                </p>
                <p className="text-indigo-600 text-sm">
                  Visibility: {resultData.visibility1}%
                </p>
              </div>
            )}
            {photo2 && (
              <div className="border border-indigo-400 rounded-lg p-6 text-center bg-white shadow-md w-72">
                ...
                <p className="text-indigo-600 text-sm">
                  Extracted Text: {resultData.detected_number2}
                </p>
                <p className="text-indigo-600 text-sm">
                  Visibility: {resultData.visibility2}%
                </p>
              </div>
            )}
          </div>
          ...
          <div className="border border-indigo-600 rounded-xl p-6 text-center bg-white shadow-lg w-80">
            ...
            <p className="text-indigo-900 text-lg font-medium">
              Matched Text: {resultData.predicted_number_plate}
            </p>
          </div>
        </div>
      )}
...
const Loader = () => (
  <div className="flex flex-col items-center justify-center space-y-3">
    <div
      className="w-7 h-7 border-4 border-indigo-300 border-t-indigo-600 rounded-full"
